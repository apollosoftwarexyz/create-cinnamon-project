...
export async function main(rawArgs: string[]) {
    ...
    let targetDirectory = await resolveIntendedDirectory(args);

    // If the target directory is undesirable, skip project creation.
    let isUndesirableReason = await isProbablyUndesirableDirectory(targetDirectory.path);
    ...
    let progress = new AnimatedBottomBar(
        'Configuring your new Cinnamon project, please wait...',
    );
    progress.open();

    // Extract to the temporary directory.
    let writtenFiles = await project.extractTemplateToTemporaryDirectory(TEMPLATES_DIRECTORY);
